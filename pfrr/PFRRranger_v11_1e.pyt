    ...
    def updateParameters(self, parameters):
        ...
        if not parameters[1].altered:
            ...
            if parameters[2].value < 0:
                ...
                parameters[11].value = str(int(dx)) + " " + str(int(mx)) + " " + str(sx)
                parameters[12].value = str(int(dy)) + " " + str(int(my)) + " " + str(sy)
        ...
        if parameters[1].altered:
            if parameters[1].value:
                ...
                if parameters[11].altered:
                    ...
                    secx = float(parameters[11].valueAsText.split(" ")[2])
                    parameters[2].value = (degx + (minx/60 + secx/3600))*-1

                if parameters[12].altered:
                    degy = int(parameters[12].valueAsText.split(" ")[0])
                    miny = float(parameters[12].valueAsText.split(" ")[1])
                    secy = float(parameters[12].valueAsText.split(" ")[2])
