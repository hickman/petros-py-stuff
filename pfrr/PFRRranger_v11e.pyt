...
class PFRR(object):
    ...
    def getParameterInfo(self):
        ...
        param11 = arcpy.Parameter(
            displayName = "Degrees Minutes Seconds - Longitude, Longitude",
            name = "dmsLon",
            datatype = "GPString",
            parameterType = "Optional",
            direction = "Input")
    ...
    def updateParameters(self, parameters):
        ...
        if not parameters[1].altered:
            ...
            if parameters[2].value < 0:
                ...
                parameters[11].value = str(int(dx)) + " " + str(int(mx)) + " " + str(sx) + ", " + str(int(dy)) + " " + str(int(my)) + " " + str(sy)
        ...
        if parameters[1].altered:
            if parameters[1].value:
                ...
                if parameters[11].altered:
                    degx = int(parameters[11].valueAsText.split(" ")[0])*-1
                    minx = float(parameters[11].valueAsText.split(" ")[1])
                    secx = float(parameters[11].valueAsText.split(" ")[2].rstrip(','))
                    degy = int(parameters[11].valueAsText.split(" ")[3])
                    miny = float(parameters[11].valueAsText.split(" ")[4])
                    secy = float(parameters[11].valueAsText.split(" ")[5])

                    parameters[2].value = (degx + (minx/60 + secx/3600))*-1
                    parameters[3].value = degy + (miny/60 + secy/3600)
                    parameters[4].value = degx
                    parameters[5].value = minx
                    parameters[6].value = secx
                    parameters[7].value = degy
                    parameters[8].value = miny
                    parameters[9].value = secy
##                parameters[2].enabled = 0
##                parameters[3].enabled = 0
...
##
##                if parameters[2].value < 0:
##                    parameters[11].value = "W " + str(int(d1x)*-1) + u'\xb0' + " " + str(int(m1x)) + "' " + str(s1x) + '", ' + "N " + str(d1y) + u'\xb0' + " " + str(int(m1y)) + "' " + str(s1y) + '"'
